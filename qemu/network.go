...
//   - MACアドレスを変更する
func (q Qemu) AttachTap(label, tap string, mac net.HardwareAddr, vcpus uint32) error {
	netdevID := fmt.Sprintf("netdev-%s", label)
	devID := fmt.Sprintf("virtio-net-%s", label)

	// check to create netdev

	if err := q.tapNetdevAdd(netdevID, tap, vcpus); err != nil {
		return fmt.Errorf("Failed to run netdev_add: err='%s'", err.Error())
	}

	if err := q.virtioNetPCIAdd(devID, netdevID, mac, vcpus); err != nil {
...
func (q *Qemu) tapNetdevAdd(id, ifname string, vcpus uint32) error {
	cmd := struct {
		ID         string `json:"id"`
